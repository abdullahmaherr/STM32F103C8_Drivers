...
void (* gp_USART_ISR_CallBack[3])(void);
...
void MCAL_USART_Init(USART_TypeDef* USARTx,USART_Config_t* p_USART_Config)
{
	...
	/* Enable USART TX/RX */
	(USARTx->CR1) |= (p_USART_Config->USART_Mode);

	/* Configure The USART Word Length */
	(USARTx->CR1) |= (p_USART_Config->USART_WordLength);

	/* Configure The USART Parity Bit */
	(USARTx->CR1) |= (p_USART_Config->USART_ParityBit);

	/* Configure The USART Stop Bits */
	(USARTx->CR2) |= (p_USART_Config->USART_StopBits);

	/* Configure The USART Flow Control */
	(USARTx->CR3) |= (p_USART_Config->USART_FlowControl);

	/* Configure The USART Baud Rate */
	BRR = BRR_REG(PCLK,p_USART_Config->USART_BaudRate);
	USARTx->BRR = BRR;
	...
	if(USART_IRQ_DISABLE != p_USART_Config->USART_IRQ)
	{
		if(USART1 == USARTx)
		{
			/* Assign CallBack Function With ISR */
			gp_USART_ISR_CallBack[0] = p_USART_Config->p_USART_ISR;
			/* NVIC IRQ Enable  */
			NVIC_USART1_EN();
		}
		else if(USART2 == USARTx)
		{
			/* Assign CallBack Function With ISR */
			gp_USART_ISR_CallBack[1] = p_USART_Config->p_USART_ISR;
			/* NVIC IRQ Enable  */
			NVIC_USART2_EN();
		}
		else if(USART3 == USARTx)
		{
			/* Assign CallBack Function With ISR */
			gp_USART_ISR_CallBack[2] = p_USART_Config->p_USART_ISR;
			/* NVIC IRQ Enable  */
			NVIC_USART3_EN();
		}
		/* IRQ MASK Enable */
		(USARTx->CR1) |= (p_USART_Config->USART_IRQ);
	}
...
{
	if(POLLING_ENABLED == a_PollingEn)
...
void USART1_IRQHandler(void)
{
	...
	if(gp_USART_ISR_CallBack[0] != NULL_PTR)
		(*gp_USART_ISR_CallBack[0])();
}

void USART2_IRQHandler(void)
{
	/* Call The ISR CallBack Function */
	if(gp_USART_ISR_CallBack[1] != NULL_PTR)
		(*gp_USART_ISR_CallBack[1])();
}

void USART3_IRQHandler(void)
{
	/* Call The ISR CallBack Function */
	if(gp_USART_ISR_CallBack[2] != NULL_PTR)
		(*gp_USART_ISR_CallBack[2])();
}
